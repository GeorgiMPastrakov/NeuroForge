...
Tensor Sigmoid::backward(const Tensor& grad_output) {
    if (!cached_output_.has_value()) {
        throw std::invalid_argument("Sigmoid backward requires a prior forward call.");
    }

    if (grad_output.shape() != cached_output_->shape()) {
        throw std::invalid_argument("Sigmoid grad output shape mismatch: expected " + cached_output_->shape().toString() + ", got " + grad_output.shape().toString() + ".");
    }
    ...
    for (size_t index = 0; index < grad_output.size(); ++index) {
        ...
        result.data()[index] = grad_output.data()[index] * value * (1.0 - value);
    }
